    ...
    try {
      await sequelize.transaction(async t => {
        for (let i = 0; i < 10000; i++) {
          let participante_escolhido =
            participantes[Utils.getRandomInt(0, participantes.length - 1)];
          let artigo_escolhido =
            artigos[Utils.getRandomInt(0, artigos.length - 1)];

          let artigo = await Artigo.findByPk(artigo_escolhido.id);
          let participante = await Participante.findByPk(
            participante_escolhido.id
          );
